...
Use reletive path, so people download from github can also access the
dataset. ./ means start from this github “data_wargling” folder. Since
we don’t use other function in janitor, so that we don’t need to write
library(janitor).
...
skip=10 means skipping 10 rows. col.names=FALSE is to make up colum
names as x1 x2 x3… rather than the origenal colum name
